...
interface Props {
    ...
    children?: ReactNode;
    onConfirm?: (value: NewSafetyModalFormValues | any) => void;
}

const NewSafetyModalForm = forwardRef<NewSafetyDetailsModalHandle, Props>(
    (
        {
            onConfirm,
            open = false,
                ...
                <Modal isOpen={Boolean(open)} onClose={() => { setOpen(false); onClose() }}>
                    <div className="">
                        <div className="text-xl text-[#1D2530] font-medium pb-3 dark:text-white">
                            {modalTypeIsNew ? "Update" : "New"} Safety Topic Details
                        </div>
