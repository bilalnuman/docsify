...
<<<<<<< HEAD
              data?.results.length == 0 ?
                ...
                data?.results?.map((m: Member) => (
                  <div
                    key={m.id}
                    className="rounded-xl sm:items-center items-start border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1A1B20] px-4 py-3 flex justify-between sm:flex-row flex-col"
                  >
                    {/* Left */}
...
<<<<<<< HEAD
                    <div className="flex sm:items-center gap-4 sm:mt-0 mt-5 sm:w-fit w-full ms-auto">
                      <div className="text-right sm:block flex items-center gap-3 flex-1 justify-between">
                        <div className="relative inline-block">
                          {
                            m?.status?.toLowerCase() === "pending" ? <>
                              <div className="capitalize text-red-500 text-xs border-red-500 rounded-full border p-1">pending invite</div>
                            </> :
                          ...
                          {
                            m?.status?.toLowerCase() !== "pending" && openMenuId === m.id && (
                              <div
                        ...
                        {m?.status?.toLowerCase() !== "pending" &&
                          <div className="text-xs text-slate-400 dark:text-gray-400 mt-1 sm:order-1 -order-1">
