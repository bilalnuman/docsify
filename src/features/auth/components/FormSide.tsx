...
const FormSide = () => {
    return (
        <div className='hidden sm:flex flex-col items-center justify-center col-span-6 px-4 dark:bg-[#2C2D34] h-full'>
            <div className='max-w-[516px] mx-auto'>
                <div className='flex items-center gap-1.5 justify-center'>
                    {Array.from({ length: 5 }).map((_, index) => (<FaStar key={index} className='text-[#FAB515]' />))}
                </div>
                ...
                <div className='flex flex-col items-center justify-center '>
                    <div className='w-[72px] h-[72px] rounded-full'>
                        <img src={avatar} alt="Avatar" className='rounded-full h-full w-full object-cover' />
                    </div>
                    <div className='text-xl font-semibold text-[#1D2530D9] dark:text-white mt-2'>Carlos Cruz</div>
