...
const InputField = React.forwardRef<HTMLInputElement, InputFieldProps>(
    ({ label, name, type = 'text', error, value, onChange, onBlur, icon, classNames, fieldMeta, iconPosition = "right", enablePasswordToggle = false, ...rest }, ref) => {

        const [secure, setSecure] = useState<boolean>(false);
        return (
            <div className={clsx("mb-4 w-full", classNames?.container)}>
                <label htmlFor={name} className={clsx("block text-sm font-medium text-[#1D2530] mb-1", classNames?.label)}>
                    {label}
                </label>
                <div className='relative w-full'>
                    {icon && <span className={`absolute top-1/2 -translate-y-1/2 z-10 ${iconPosition === 'left' ? "start-3" : "end-3"} ${classNames?.iconClass}`}>{icon}</span>}
                    <input
                        {...fieldMeta}
                        id={name}
